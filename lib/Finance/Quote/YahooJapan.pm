...
sub _scrape {
    ...
    if (defined $container) {
        ...
        for my $e ($container->find('article')) {
            my ($sym_elm) = $e->look_down('class', qr/\bSearchItem__supplement__/);
            my $sym = $sym_elm->as_text;
            my ($date, $time) = _parse_datetime($e->find('time')->as_text);
            my $quote = {
                name  => $e->find('h2')->as_text,
                price => $e->look_down('class', qr/\bSearchItem__price__/)->look_down('class', qr/\bStyledNumber__value__/)->as_text,
                date  => $date,
                time  => $time
            };
